...
}elseif($_POST['subaction'] == "active")
{
	foreach($_POST[delete] as $k=>$v)
	{
		$select = "update request_info set archieve=1 WHERE mid='$k'";
		$result = mysql_query($select) or die(mysql_error());
		$pid = "";
	}
}
...
echo "<table align=center border=1 width=800>
	<tr><td colspan='10'  align='right'  ><a href='admin.php?action=request_archieve'>Archive List</a></td></tr>
	<tr><th>Archive</th><th>Name</th><th>Business<th>Email</th><th>Phone</th><th>Date Created</th></tr>

	";
$select = "SELECT * FROM request_info where archieve='0' order by mid desc ";
...
$result = mysql_query($select) or die(mysql_error());
	...
	echo "<tr><td><input type=submit value='Archive' onclick=fun_redirect('1') ></td><td colspan=5><input type=submit value='Delete' onclick=fun_redirect('2') ></td></tr>";
	echo "</table></form>";
	?>
<script>
